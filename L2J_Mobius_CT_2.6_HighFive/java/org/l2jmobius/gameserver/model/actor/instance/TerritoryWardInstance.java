...
public class TerritoryWardInstance extends Attackable
{
	 ...
	 */
	public TerritoryWardInstance(NpcTemplate template)
	{
		super(template);
	...
	public void onSpawn()
	{
		...
		if (getCastle() == null)
		{
			LOGGER.warning("TerritoryWardInstance(" + getName() + ") spawned outside Castle Zone!");
		}
	}
	...
	public boolean doDie(Creature killer)
	{
		...
		if (killer.isPlayer())
		{
			if ((((PlayerInstance) killer).getSiegeSide() > 0) && !((PlayerInstance) killer).isCombatFlagEquipped())
			{
				((PlayerInstance) killer).addItem("Pickup", getId() - 23012, 1, null, false);
			}
			else
			{
